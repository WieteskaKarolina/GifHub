...
def recommend_similar_gifs(user_id, top_n=5):
    cur.execute("SELECT tags.tag_name FROM tags JOIN gif_tags ON tags.tag_id = gif_tags.tag_id JOIN gifs ON gif_tags.gif_id = gifs.gif_id WHERE gifs.user_id = %s", (user_id,))
    rows = cur.fetchall()
